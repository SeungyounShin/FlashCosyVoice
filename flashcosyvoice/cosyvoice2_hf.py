...
def build_input_ids(
    ...
    # Text-only conditioning for LLM; Flow will use prompt_wav separately
    offset = speech_vocab_size + 2
    text_ids = [tid + offset for tid in tokenizer.encode(text)]
    # [<speech_sos>] + text + [<speech_eos>]
    return [speech_vocab_size] + text_ids + [speech_vocab_size + 1]
...
def main():
    ...
    # Flow -> mel -> HiFi-GAN -> wav (use only generated speech tokens; prompt_wav biases via prompt_mel + spk_emb)
    flow_inputs = torch.tensor(out_ids).unsqueeze(0)
    flow_inputs_lens = torch.tensor([len(out_ids)], dtype=torch.int32)

    print(f"flow_inputs shape               : {flow_inputs.shape}")
    print(f"flow_inputs_lens shape          : {flow_inputs_lens}")
    print(f"prompt_mels_for_flow shape      : {prompt_mels_for_flow.shape}")
    print(f"prompt_mels_lens_for_flow shape : {prompt_mels_lens_for_flow}")
    print(f"spk_emb_for_flow shape          : {spk_emb_for_flow.shape}")
    ...
    with torch.amp.autocast("cuda", dtype=torch.float16 if (cfg.hf_config.fp16_flow or args.fp16_flow) else torch.float32):
        batch_generated_mels, batch_generated_mels_lens = flow(
            flow_inputs.cuda(), flow_inputs_lens.cuda(),prompt_mels_for_flow.cuda(), 
            prompt_mels_lens_for_flow.cuda(), spk_emb_for_flow.cuda(), 
            streaming=False, finalize=True
        )

    # 기존 (잘못된 슬라이싱)
    mel = batch_generated_mels[0, :, prompt_mels_lens_for_flow[0].item():batch_generated_mels_lens[0].item()].unsqueeze(0)
    print("mel shape:", mel.shape)  # 확인용
